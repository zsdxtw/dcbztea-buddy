...
export default function InventorySettings() {
  ...
  const openAdd = () => {
    setEditing(null);
    setForm({ ...emptyForm, code: `WH-${String(ownList.length + 1).padStart(2, '0')}` });
    setShowDrawer(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editing) {
      ...
      const newWh: Warehouse = { ...form, id: `wh-own-${Date.now()}`, category: 'own' };
          ...
          {(['own', 'partner'] as TabKey[]).map(k => (
            ...
            >
              ...
              <span style={{ marginLeft: 6, fontSize: 'var(--text-xs)', color: 'var(--color-neutral-400)' }}>
                {k === 'own' ? ownList.length : partnerList.length}
              </span>
            </button>
          ))}
        </div>
