  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-4)' }}>
        ...
        {deleteMode ? (
          <>
            <Button style={{ background: '#FF0086', borderColor: '#FF0086' }} onClick={() => setShowDeleteConfirm(true)} disabled={selectedForDelete.size === 0}>删除所选({selectedForDelete.size})</Button>
            <Button variant="ghost" onClick={exitDeleteMode}>取消</Button>
          </>
        ) : (
          <Button style={{ background: '#FF0086', borderColor: '#FF0086' }} onClick={enterDeleteMode}>删除</Button>
        )}
        ...
        <div className="category-dialog-overlay" onClick={() => setShowDeleteConfirm(false)}>
          <div className="category-dialog" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-2)' }}>
              ...
              <Button style={{ background: '#FF0086', borderColor: '#FF0086' }} onClick={confirmDelete}>确认删除</Button>
            </div>
          </div>
        </div>
      )}
